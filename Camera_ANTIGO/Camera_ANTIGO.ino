...
void setup(void)
{
    ...
    pinModeFast(SIpin, OUTPUT);
    pinModeFast(CLKpin, OUTPUT);
    //pinMode (AOpin, INPUT);

    pinModeFast(sensor_cruzamento, INPUT);
    pinModeFast(led, OUTPUT);
    pinModeFast(botao, INPUT_PULLUP);
    pinMode(rightMotor, OUTPUT);
    pinMode(leftMotor, OUTPUT);
    pinMode(rightGnd, OUTPUT);
    pinMode(leftGnd, OUTPUT);

    digitalWriteFast(SIpin, LOW);  // IDLE state
    digitalWriteFast(CLKpin, LOW); // IDLE state
    ...
    digitalWriteFast(rightMotor, LOW);
    digitalWriteFast(rightGnd, LOW);
    //digitalWrite(leftMotor, HIGH);
    digitalWriteFast(leftMotor, LOW);
    digitalWriteFast(leftGnd, LOW);
    delay(3000);
    calibra();
