...
FrescaleCamera camera = FrescaleCamera(CAM_AOUT, CAM_SI, CAM_CLK);
...
void loop()
{
    ...
    //RECEBE INFORMAÇÃO DA CÂMERA (Array de 128 posições)
    int *leituraCamera;
    leituraCamera = camera.leSensor();
...
void movimenta(unsigned char velocEsquerda, unsigned char velocDireita)
{
    ...
    if (velocEsquerda < MIN_SPEED_ESQUERDA)
        velocEsquerda = MIN_SPEED_ESQUERDA;
    else if (velocEsquerda > MAX_SPEED_ESQUERDA)
        velocEsquerda = MAX_SPEED_ESQUERDA;
    if (velocDireita < MIN_SPEED_DIREITA)
        velocDireita = MIN_SPEED_DIREITA;
    else if (velocDireita > MAX_SPEED_DIREITA)
        velocDireita = MAX_SPEED_DIREITA;
